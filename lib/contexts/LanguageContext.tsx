...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    setIsClient(true);
    // Carregar idioma do localStorage após montagem no cliente
  ...
  const t = (key: string): string => {
    // Durante SSR e antes da hidratação, sempre usar português
    const currentLanguage = isClient ? language : 'pt';
    return translations[currentLanguage][key] || key;
  };

  // Sempre fornecer o contexto, mesmo antes de montar
  // Durante SSR, sempre usar 'pt' para evitar mismatch de hidratação
  const value: LanguageContextType = {
    language: isClient ? language : 'pt',
    setLanguage,
    t,
  };
