      ...
      {!isEditing ? (
        ...
        <div className="py-3 px-3" style={{ background: 'rgba(255,255,255,0.04)' }}>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="col-span-2">{inp(editForm.titulo_do_premio, v => setEditForm({ ...editForm, titulo_do_premio: v }), 'Título do prêmio *')}</div>
            <div>{inp(editForm.tipo, v => setEditForm({ ...editForm, tipo: v }), 'Tipo (ganhou, indicado…)')}</div>
            <div>{inp(editForm.categoria, v => setEditForm({ ...editForm, categoria: v }), 'Categoria')}</div>
            <div>{inp(editForm.festival_nome, v => setEditForm({ ...editForm, festival_nome: v }), 'Festival')}</div>
...
export function PremiacoesPanel({ filmeId }: { filmeId: string }) {
  ...
  const handleEditStart = (r: Premiacao) => {
    setEditingId(r.id)
    setEditError(null)
    setEditForm({
      titulo_do_premio: r.titulo_do_premio,
      tipo: r.tipo ?? '',
      categoria: r.categoria ?? '',
      festival_nome: r.festival_nome ?? '',
      ano: r.ano?.toString() ?? '',
      observacoes: '',
    })
    setAdding(false)
  ...
  const handleEditSave = async () => {
    if (!editForm.titulo_do_premio.trim()) { setEditError('Título do prêmio é obrigatório'); return }
    setEditSaving(true); setEditError(null)
    try {
      const payload = {
        titulo_do_premio: editForm.titulo_do_premio,
        tipo: editForm.tipo || null,
        categoria: editForm.categoria || null,
        festival_nome: editForm.festival_nome || null,
        ano: editForm.ano ? parseInt(editForm.ano) : null,
        observacoes: editForm.observacoes || null,
      }
      const res = await fetch(`/api/admin/filmes/${filmeId}/premiacoes/${editingId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
