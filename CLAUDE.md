...
| Formulário de contato não envia | a Vercel não tem `RESEND_API_KEY`. Até 11/09/2026 a rota respondia "enviada com sucesso" e a mensagem se perdia; agora responde 503 e aponta contato@moveofilmes.com. Para ativar: conta no Resend, domínio verificado, `RESEND_API_KEY` (e `EMAIL_FROM`/`EMAIL_TO`, se não forem contato@) nos três ambientes |
| Filmes sem `categoria_site` | três, e por isso fora de qualquer listagem: Feijão com Arroz, O Colar de Coralina e Uma Dose Violenta de Qualquer Coisa (sem ano, sinopse nem imagem). Os outros nove foram para `cinema` em 10/09/2026 — backup em `../backups/2026-09-10/categorias-antes.json`. Não Há Magia, que era rascunho, foi publicado em `cinema` no mesmo dia, quando entrou na home — backup em `../backups/2026-09-10/nao-ha-magia-antes-de-publicar.json` |
